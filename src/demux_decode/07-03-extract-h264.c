...
int main(int argc, char **argv)
{
    ...
    AVBSFContext *bsf_ctx = NULL; // 像类似AVBSFContext这样context（上下文）都是用于存储某个东西的各种内容。如AVBSFContext就是存储bsfilter的各种内容
    // 2 初始化过滤器上下文
    ...
    while (0 == file_end)
    {
        if ((ret = av_read_frame(ifmt_ctx, pkt)) < 0) // 记得利用av_packet_unref(pkt);减少引用数
        {
            // 没有更多包可读
            file_end = 1;
        ...
        if (ret == 0 && pkt->stream_index == videoindex)
        {
            ...
            // av_bsf_send_packet:提交一个数据包进行过滤。将pkt提交进过滤器进行处理
            if (av_bsf_send_packet(bsf_ctx, pkt) != 0) 
            {
                av_packet_unref(pkt);   // 发送完了就不用了就把资源释放掉
                continue;       // 继续送
            }
            av_packet_unref(pkt);   // 发送完了就不用了就把资源释放掉
            while(av_bsf_receive_packet(bsf_ctx, pkt) == 0)   // 接收经过过滤器处理后的AVPacket，放入pkt中。
            {
                out_pkt_count++;
                // printf("fwrite size:%d\n", pkt->size);
                size_t size = fwrite(pkt->data, 1, pkt->size, outfp);   // 将经过过滤器处理后的pkt写入outfp
                if(size != pkt->size)
                {
                    printf("fwrite failed-> write:%u, pkt_size:%u\n", size, pkt->size);
                }
                av_packet_unref(pkt); // 接收完了就不用了就把资源释放掉
            }
            if(out_pkt_count >= 2)  // 大于二，说明这个pkt分了二次及以上进行receive
            {
                printf("cur pkt(size:%d) only get 1 out pkt, it get %d pkts\n",
                       input_size, out_pkt_count);
            }
#else // 不经过过滤器直接写入outfp中   // TS流可以直接写入
