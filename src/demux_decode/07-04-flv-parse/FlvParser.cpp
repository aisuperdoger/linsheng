...
int CFlvParser::Parse(uint8_t *pBuf, int nBufSize, int &nUsedLen) // pBuf指向的是整个flv文件
{
    int nOffset = 0;  // 用于记录解析到什么位置了，刚开始当然是零，表示从头开始解析。

    if (_pFlvHeader == 0) // 将flv文件中的flv header提取出来
    {
        CheckBuffer(9);
    ...
    while (1)
    {
        CheckBuffer(15);   // nPrevSize(4字节) + Tag header(11字节)。注：flv body是由一个个nPrevSize+Tag header组成
        int nPrevSize = ShowU32(pBuf + nOffset);  // U32代表读取4个字节（32位）
        nOffset += 4;
        ...
        _vpTag.push_back(pTag);  // 将整个tag放入一个列表中
    }
...
CFlvParser::FlvHeader *CFlvParser::CreateFlvHeader(uint8_t *pBuf) // pBuf就相当于输入的flv文件
{
    ...
    memcpy(pHeader->pFlvHeader, pBuf, pHeader->nHeadSize); // 将头部拷贝一份到pHeader->pFlvHeader中
...
void CFlvParser::Tag::Init(TagHeader *pHeader, uint8_t *pBuf, int nLeftLen)
{
    ...
    memcpy(_pTagHeader, pBuf, 11);      // 复制tag头部
    // 复制标签 body
...
CFlvParser::CVideoTag::CVideoTag(TagHeader *pHeader, uint8_t *pBuf, int nLeftLen, CFlvParser *pParser)
{
    ...
    uint8_t *pd = _pTagData;             // _pTagData为tag body
    _nFrameType = (pd[0] & 0xf0) >> 4;  // 帧类型
    _nCodecID = pd[0] & 0x0f;           // 视频编码类型
    // 开始解析
    if (_header.nType == 0x09 && _nCodecID == 7)  // 只对H264进行解析
    {
...
int CFlvParser::CAudioTag::ParseAACTag(CFlvParser *pParser)
{
    ...
    {
        // 解析配置信息
        ParseAudioSpecificConfig(pParser, pd); // 解析AudioSpecificConfig。获取到的配置信息将在下面的ParseRawAAC(pParser, pd);中使用
    }
    // 如果是音频数据
...
CFlvParser::Tag *CFlvParser::CreateTag(uint8_t *pBuf, int nLeftLen)
{
    ...
    header.nTotalTS = (uint32_t)((header.nTSEx << 24)) + header.nTimeStamp; // << 24将扩展字段移动到高8位处
    // 标签头部解析结束
...
int CFlvParser::CVideoTag::ParseH264Tag(CFlvParser *pParser)
{
    ...
    */
    int nAVCPacketType = pd[1];  // tag body的第二个字节
...
AVCDecoderConfigurationRecord {
    ...
    bit(6) reserved = ‘111111’b;            [4 第4个字节]
    uint32_t(2) lengthSizeMinusOne;         [4 第4个字节] 计算方法是 1 + (lengthSizeMinusOne & 3)，实际计算结果一直是4
...
{
    uint8_t *pd = pTagData;
    // 跨过 Tag Data的VIDEODATA(1字节) AVCVIDEOPACKET(AVCPacketType(1字节) 和CompositionTime(3字节))
    // 总共跨过5个字节，从第六个字节开始才是AVC的配置信息（AVCDecoderConfigurationRecord）
    ...
    _pMedia = new uint8_t[_nMediaLen];    // _pMedia指向标签的元数据，改造后的数据，即添加了start code后的数据
    // 保存元数据，这里应该是给sps和pps添加start code
